...
| Observability | Catalog/value files codify `monitoring-crds`, `monitoring`, `node-exporter`, `loki`, `alloy`, `push`/ntfy, and `apprise`, plus chart-native metrics integrations for current platform/base services, including Authentik. `terraform/push` owns generated ntfy users/tokens/ACL config, the Alertmanager ntfy publisher token, empty Apprise destination config, and the mobile client Secret. Alertmanager routes low/medium/high/critical alerts directly to ntfy; Apprise is reserved for future non-ntfy fanout and is not in the active alert route. Observability PVCs are disposable readiness state, not backup targets. | Sync and prove notification delivery with one synthetic Alertmanager alert and one ArgoCD failure/degraded condition. |
...
| Mail relay | `argocd/catalog/platform/mail-relay.yaml` deploys a cluster-internal Maddy submission relay in the `mail` namespace with a local PGP gateway sidecar. External Secrets reads upstream SMTP2GO credentials from SSM for the sidecar and the shared app-to-relay sender credential from SSM for Maddy. Maddy authenticates the single relay user, authorizes it for any sender localpart under `d-reis.com`, and hands mail to `127.0.0.1:2525`; the sidecar encrypts and relays to SMTP2GO. Vaultwarden and Authentik receive namespace-local copies of the sender credential. The service ships a Grafana dashboard for PGP gateway counters plus Maddy/sidecar pod health and logs. | Needs a live sync/test after the PGP recipient rule and public key are configured. SMTP2GO must accept the `@d-reis.com` sender localparts used by workloads. |
...
- Apprise API for future non-ntfy notification fanout.
...
- Add ntfy as the active observability notification path.
- Keep Apprise API available for future non-ntfy fanout, but do not route
  Alertmanager alerts through it while its destination files are empty.
...
- One synthetic Alertmanager alert reaches ntfy directly.
...
- Odoo adoption.
- Narrow backup credential blast radius with per-app rsync.net subaccounts or
  another per-prefix authorization boundary, plus per-repository Restic
  passwords. The current VolSync wiring is acceptable for initial readiness but
  should not remain the long-term isolation model.
